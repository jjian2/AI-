...
/* ===================================================
   [최상단 배치] 로그인 및 회원가입 페이지 전용 제어 로직 (기능 고도화)
   =================================================== */
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 비밀번호 확인 입력창에 입력할 때마다 검사
  if (signupPw && signupPwConfirm) {
      signupPwConfirm.addEventListener("input", checkPasswordMatch);
      signupPw.addEventListener("input", checkPasswordMatch);
  }

  // 아이디 중복확인 여부를 체크하는 변수 (false면 가입 진행 막기)
  ...
  if (openSignupBtn && signupModal) {
    console.log("로그인 페이지 감지 - 고도화 기능 연결 완료");
    ...
    // 모달 닫기 및 초기화
    const closeElements = [closeSignupBtn, backToLoginBtn];
    closeElements.forEach(btn => {
      if (btn) {
        btn.addEventListener("click", function () {
          signupModal.classList.remove("open");
          if (signupForm) signupForm.reset();
          if (pwMismatchMsg) pwMismatchMsg.textContent = "";
    ...
    // [기능 1] 아이디 중복확인 버튼 비동기 통신
    const checkIdBtn = document.getElementById("checkIdBtn");
    const usernameInput = document.getElementById("signupId");

    if (checkIdBtn && usernameInput) {
      checkIdBtn.addEventListener("click", function () {
        ...
        // 스프링 부트 서버의 중복 체크 API 호출
        fetch(`/api/user/check-id?username=${encodeURIComponent(username)}`)
    ...
    // [기능 3] 회원가입 폼 전송 시 최종 검증 (비밀번호 일치 + 중복확인 여부)
    if (signupForm) {
      signupForm.addEventListener("submit", function (e) {
        // 비밀번호 일치 검사
        if (signupPw && signupPwConfirm && signupPw.value !== signupPwConfirm.value) {
          e.preventDefault();
          if (pwMismatchMsg) pwMismatchMsg.textContent = "비밀번호가 일치하지 않습니다.";
  ...
  // ---------------------------------------------------
	...
	// ---------------------------------------------------
	  // 로그인 실패 처리 및 3회 이상 실패 시 회원가입 유도
	  // ---------------------------------------------------
	  if (errorType === 'loginfail') {
	    if (loginErrorMsg) {
	      loginErrorMsg.textContent = "아이디 또는 비밀번호가 올바르지 않습니다.";
...
/* ===================================================
   여기서부터 기존 대시보드용 main.js 코드가 시작됩니다.
   =================================================== */
/* ===== 사이드바 네비게이션 (지금은 화면 전환 없이 활성 표시만) ===== */
// ... 기존 코드 (const navItems = ...) 이어서 쭉 배치
